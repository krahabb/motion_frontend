...
class BoolParam(Param):
    ...
    def __new__(cls, value, descriptor: 'Descriptor'):
        ...
        _self = super().__new__(
            cls,
            value if isstr else VALUE_ON if boolvalue else VALUE_OFF,
            descriptor
            )
...
class Descriptor:
    ...
    def __init__(self, _builder: Param, _validator: Any = None, _set: frozenset = None):
        self.builder: Param = _builder
        self.set: frozenset = _set
...
def DESCRIPTOR_INT_RANGE(min: int = None, max: int = None) -> Descriptor:
    return Descriptor(IntParam, _validator=vol.Range(min=min, max=max))
def DESCRIPTOR_INT_ENUM(_set: frozenset) -> Descriptor:
...
def DESCRIPTOR_STRING_ENUM(_set: frozenset) -> Descriptor:
    return Descriptor(Param, _set=_set)
def DESCRIPTOR_UPPERSTRING_ENUM(_set: frozenset) -> Descriptor:
    return Descriptor(UpperStringParam, _set=_set)


def build_value(key: str, value: Any) -> Param:
    """
...
SECTION_IMAGE_MAP = {
    ...
    LOCATE_MOTION_STYLE: DESCRIPTOR_STRING_ENUM((VALUE_BOX, VALUE_REDBOX, VALUE_CROSS, VALUE_REDCROSS)),
...
SECTION_PICTURE_MAP = {
    PICTURE_OUTPUT: DESCRIPTOR_STRING_ENUM((VALUE_OFF, VALUE_ON, VALUE_FIRST, VALUE_BEST)),
    ...
    OUTPUT_PICTURES: DESCRIPTOR_STRING_ENUM((VALUE_OFF, VALUE_ON, VALUE_FIRST, VALUE_BEST)),
...
SECTION_SET_MAP: MappingProxyType[str, MappingProxyType[str, Descriptor]] = {
...
CAMERACONFIG_SET = (
    ...
    MMALCAM_NAME, MMALCAM_CONTROL_PARAMS,
)
...
RESTARTCONFIG_SET = (
    ...
    VIDEO_DEVICE, VIDEODEVICE, NETCAM_URL,
    MMALCAM_NAME, MMALCAM_CONTROL_PARAMS,
)

SCHEMA: MappingProxyType[str, Descriptor] = ChainMap(
    SECTION_SYSTEM_MAP,
    SECTION_V4L2_MAP,
    SECTION_NETCAM_MAP,
    SECTION_MMALCAM_MAP,

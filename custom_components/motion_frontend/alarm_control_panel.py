...
from homeassistant.components.alarm_control_panel.const import (
    ...
    SUPPORT_ALARM_ARM_CUSTOM_BYPASS, SUPPORT_ALARM_ARM_NIGHT
)
from homeassistant.const import (
    ...
    STATE_PAUSED, STATE_PROBLEM,
)

from .motionclient import MotionHttpClient
...
from .const import (
    ...
    EXTRA_ATTR_LAST_TRIGGERED, EXTRA_ATTR_LAST_PROBLEM,
)


#Mock MotionFrontendApi
...
class MotionFrontendAlarmControlPanel(AlarmControlPanelEntity):
    ...
    def __init__(self, api: MotionFrontendApi):
        ...
        self._disarmhome_cameras: frozenset = frozenset(data.get(CONF_ALARM_DISARMHOME_CAMERAS, []))
        self._disarmaway_cameras: frozenset = frozenset(data.get(CONF_ALARM_DISARMAWAY_CAMERAS, []))
        self._disarmnight_cameras: frozenset = frozenset(data.get(CONF_ALARM_DISARMNIGHT_CAMERAS, []))
        self._disarmbypass_cameras: frozenset = frozenset(data.get(CONF_ALARM_DISARMBYPASS_CAMERAS, []))
        self._disarmed_cameras: frozenset = None
        """
        ...
        if self._pause_disarmed: # the set of disarmed cameras should match with some of our state-sets
    ...
    def code_format(self):
        if self._pin:
            return FORMAT_NUMBER if self._pin.isnumeric() else FORMAT_TEXT
        return None
    ...
    def notify_state_changed(self, camera: MotionFrontendCamera):
        ...
        problem = False
        for id, camera in self._api.cameras.items():
            if id in self._disarmed_cameras:
                continue
            if camera.is_triggered:
                self._set_state(STATE_ALARM_TRIGGERED)
                break
            problem |= (camera.state == STATE_PROBLEM)
        else:
